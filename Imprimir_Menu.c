...
void impressao_do_menu(){
    ...
    printf("< Olá, bem vindo ao restautante Hack 'n' Cheese >\n");
    printf(" -----------------------------------------------\n");
    printf("        \\   \n");
    printf("         \\  ─────█─▄▀█──█▀▄─█─────\n");
    printf("          \\ ────▐▌──────────▐▌────\n");
...
cardapio string_para_prato(const char* nome_do_prato) {
    ...
    if (strcmp(nome_minusculo, "camarão ao alho") == 0) return CAMARAO_AO_ALHO;
    if (strcmp(nome_minusculo, "lasanha à bolonhesa") == 0) return LASANHA_A_BOLONHESA;
    if (strcmp(nome_minusculo, "filé mignon com fritas") == 0) return FILE_MIGNON_COM_FRITAS;
    if (strcmp(nome_minusculo, "frango grelhado com legumes") == 0) return FRANGO_GRELHADO_COM_LEGUMES;
    if (strcmp(nome_minusculo, "bacalhau à gomes de sá") == 0) return BACALHAU_A_GOMES_DE_SA;
